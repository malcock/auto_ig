...
EPIC_IDS = ["CS.D.GBPUSD.TODAY.IP","CS.D.EURUSD.TODAY.IP","CS.D.USDJPY.TODAY.IP","CS.D.GBPAUD.TODAY.IP","CS.D.EURCAD.TODAY.IP",
            ...
            "CS.D.USDCAD.TODAY.IP", "CS.D.USDCHF.TODAY.IP","CS.D.EURCHF.TODAY.IP","CS.D.AUDJPY.TODAY.IP", "CS.D.GBPCAD.TODAY.IP",
            "CS.D.NZDUSD.TODAY.IP", "CS.D.GBPNZD.TODAY.IP", "CS.D.NZDJPY.TODAY.IP", "CS.D.NZDCAD.TODAY.IP", "CS.D.NZDAUD.TODAY.IP"   ]
# EPIC_IDS = ["CS.D.GBPAUD.TODAY.IP"] 
...
def fill_signals():
    ...
    for key,s in sigs.items():
        ...
        for sig in s:
            if "OPEN" in sig.name:
                output+="<li>{} - {}/{} - {} ({} <b>({})</b>) - {} unused:{} comment: {}</li>".format(sig.timestamp,sig.market,sig.resolution,sig.name,sig.position,sig.score,sig.life,sig.unused,sig.comment)
            else:
                output+="<li><small>{} - {}/{} - {} ({} <b>({})</b>) - {} unused:{} comment: {}</small></li>".format(sig.timestamp,sig.market,sig.resolution,sig.name,sig.position,sig.score,sig.life,sig.unused,sig.comment)
        output += "</ul></li>"
